...
# Robust project-root/data detection
# ============================================================

# ---- Locate AusData.csv robustly ----
possible_data <- c(
...
existing_data <- possible_data[file.exists(possible_data)]
...
possible_spd <- c(
  "C:/Users/Kevan/Documents/Kev Carbon/data/spd.csv",
  "data/spd.csv",
  "../data/spd.csv",
  "../../data/spd.csv",
  "spd.csv",
...
existing_spd <- possible_spd[file.exists(possible_spd)]
spd_path <- if (length(existing_spd)) existing_spd[1] else NA_character_

precomputed_spd <- NULL
if (!is.na(spd_path)) {
  tmp <- try(read_csv(spd_path, show_col_types = FALSE), silent = TRUE)
  if (!inherits(tmp, "try-error")) precomputed_spd <- tmp
}

# ---- Optional saved NIMBLE results ----
possible_results <- c(
  "C:/Users/Kevan/Documents/Kev Carbon/data/AusData_decline_model_fits_long.rds",
  "C:/Users/Kevan/Documents/Kev Carbon/data/AusData_decline_model_fits_medium.rds",
  "C:/Users/Kevan/Documents/Kev Carbon/data/AusData_decline_model_fits_test.rds",
...
existing_results <- possible_results[file.exists(possible_results)]
result_path <- if (length(existing_results)) existing_results[1] else NA_character_
  ...
  nav_panel(
    ...
    card(
      ...
      p(
        ...
        "and running MCMC interactively."
      ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$calculate_spd, {
    ...
    tryCatch({
      ...
      if (is.data.frame(spd_obj)) candidate <- spd_obj
      ...
      if (is.null(candidate) || ncol(candidate) < 2) {
        stop("SPD calculated but could not extract plotting columns.")
      }
      ...
      showNotification("SPD ready.", type = "message", duration = 3)
  ...
  output$spd_plot <- renderPlot({
    ...
    if (!all(c("calBP", "PrDens") %in% names(x))) {
      validate(need(ncol(x) >= 2, "spd.csv needs at least two columns."))
      x <- data.frame(
  ...
  output$nimble_summary <- renderPrint({
    if (is.null(saved_results)) {
      cat(
        ...
        "Place one of these in the data folder:\n",
        "AusData_decline_model_fits_long.rds\n",
        "AusData_decline_model_fits_medium.rds\n",
        "AusData_decline_model_fits_test.rds\n"
      )
      return()
    }
