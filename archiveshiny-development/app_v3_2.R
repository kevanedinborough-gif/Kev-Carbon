...
# RLifeHistory Remote Console v3.2
...
server <- function(input, output, session) {
  ...
  output$job_log <- renderText({
    ...
    tryCatch({
      ...
      if (!is.null(st$status) && identical(st$status, "RUNNING")) {
        "Analysis is running. Windows has temporarily locked the live log."
      } else if (!is.null(st$status) && identical(st$status, "FINISHED")) {
        "Analysis finished. The log file is temporarily locked, but the completed report is available."
      } else {
        paste("Log temporarily unavailable:", conditionMessage(e))
      }
    })
  })
