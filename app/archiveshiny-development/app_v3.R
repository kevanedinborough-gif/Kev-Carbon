...
# RLifeHistory Remote Console v3
# Runs the existing NIMBLE vignette in a SEPARATE R process.
# This means the Shiny browser can disconnect while the desktop job continues.
...
status_file <- file.path("run_status", "latest.txt")
log_file <- file.path("run_status", "latest.log")
...
ui <- page_navbar(
  ...
  nav_panel("Data",
    card(
      ...
      if (is.null(aus)) {
        p("AusData.csv not found here. The NIMBLE vignette itself uses AusData.xlsx.")
      } else {
        tagList(
  ...
  nav_panel("Run",
    layout_sidebar(
      sidebar = sidebar(
        radioButtons("run_mode", "Run length",
...
server <- function(input, output, session) {
  ...
  output$data_plot <- renderPlot({
    ...
    plot_dat <- data.frame(Index = seq_len(nrow(dat)), Age = dat[[age_col]])
    ggplot(plot_dat, aes(Index, Age)) +
      geom_point(size = 2, alpha = 0.7) +
      labs(title = paste("Using column:", age_col), x = "Record", y = age_col) +
  ...
  observeEvent(input$run_job, {
    ...
    cmd <- sprintf(
      ...
      normalizePath("run_shiny_job.R", winslash = "/", mustWork = TRUE),
      input$run_mode,
      normalizePath("run_status", winslash = "/", mustWork = TRUE) |> file.path("latest.log")
    )

    # Windows: start a detached process so closing the phone/browser does not stop it.
    if (.Platform$OS.type == "windows") {
      shell(sprintf('start "" /B cmd /c %s', cmd), wait = FALSE)
    } else {
      system(paste(cmd, "&"), wait = FALSE)
    }

    showNotification(paste("Started", input$run_mode, "analysis on desktop."), type = "message")
  })
  ...
  output$job_log <- renderText({
    ...
    if (!file.exists(log_file)) return("No job log yet.")
    x <- readLines(log_file, warn = FALSE)
    paste(tail(x, 30), collapse = "\n")
  })
  ...
  output$report_status <- renderUI({
    ...
    if (file.exists(report_file)) {
      ...
      tags$div(class = "alert alert-secondary", "No completed HTML report found yet.")
    }
  })

  output$download_report <- downloadHandler(
    filename = function() paste0("RLifeHistory_", Sys.Date(), ".html"),
